...
export default {
  async fetch(request, env) {
    ...
    if (mt === 'OPTIONS') return new Response(null, { status: 204, headers: cors() });
...
async function ba(env) {
  ...
  var d = await r.json();
  return { t: d.authorizationToken, a: d.apiUrl + '/b2api/v2', d: d.downloadUrl };
}

async function gbid(env, a) {
...
async function up(req, env) {
  ...
  var f = fd.get('file');
  if (!f) return new Response(JSON.stringify({ error: 'no file' }), { status: 400, headers: cors() });
  var code = g6();
  var ext = f.name.split('.').pop();
  var fn = code + '.' + ext;
  var buf = await f.arrayBuffer();
  var a = await ba(env);
  var bid = await gbid(env, a);
  var ur = await fetch(a.a + '/b2_get_upload_url', {
    method: 'POST', headers: { 'Authorization': a.t, 'Content-Type': 'application/json' },
    body: JSON.stringify({ bucketId: bid })
  });
  if (!ur.ok) return new Response(JSON.stringify({ error: 'url' }), { status: 500, headers: cors() });
  ...
  if (!up2.ok) return new Response(JSON.stringify({ error: 'b2' }), { status: 500, headers: cors() });
  await sx(env, '/fileshare', JSON.stringify({ code: code, filename: f.name, filesize: f.size, filetype: f.type || ext, storage_path: fn, uploaded_by: 'user' }), 'POST');
  return new Response(JSON.stringify({ success: true, code: code }), { headers: cors() });
}

async function dl(url, env) {
  var c = url.searchParams.get('code');
  if (!c) return new Response(JSON.stringify({ error: 'no code' }), { status: 400, headers: cors() });
  var r = await sx(env, '/fileshare?code=eq.' + c + '&select=*');
  var d = await r.json();
  if (!d || !d.length) return new Response(JSON.stringify({ error: 'not found' }), { status: 404, headers: cors() });
  var a = await ba(env);
  var bid = await gbid(env, a);
  ...
  return new Response(JSON.stringify({
    success: true, code: c, filename: d[0].filename, filesize: d[0].filesize,
    url: a.d + '/file/' + env.B2_BUCKET + '/' + d[0].storage_path + '?Authorization=' + encodeURIComponent(ad.authorizationToken)
  }), { headers: cors() });
}

async function ml(env) {
  var r = await sx(env, '/fileshare?order=created_at.desc');
  var d = await r.json();
  return new Response(JSON.stringify(d || []), { headers: cors() });
}

async function al(env) {
  var r = await sx(env, '/fileshare?order=created_at.desc');
  var d = await r.json();
  return new Response(JSON.stringify(d || []), { headers: cors() });
}

async function adel(url, env) {
  var id = url.searchParams.get('id');
  if (!id) return new Response(JSON.stringify({ error: 'no id' }), { status: 400, headers: cors() });
  await sx(env, '/fileshare?id=eq.' + id, null, 'DELETE');
